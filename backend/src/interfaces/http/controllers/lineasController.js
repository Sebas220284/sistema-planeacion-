...
exports.toggleActiva = async (req, res) => {
  try {
    ...
    await pool.query('UPDATE planning_templates SET activa = $1 WHERE id = $2', [activa, req.params.id]);
    res.json({ message: 'Estado de la l�nea actualizado' });
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: 'Error al cambiar el estado de la l�nea' });
  }
};
